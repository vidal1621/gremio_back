...
def maestro_pagos():
    ...
    if request.method == 'GET':
        sql_pendiente = "select * from pagos p join alumnos using (cod_usuario) where desc_pago='Pendiente'"
        cursor.execute(sql_pendiente)
        pendientes = cursor.fetchall()
        sql_pagados = "select * from pagos p join alumnos using (cod_usuario) where desc_pago='Pagado'"
        ...
        return Response(response=json.dumps(maestro_pagos), status=200, mimetype='application/json')
        ...
        return Response(response=json.dumps(maestro_pagos_usuario), status=200, mimetype='application/json')